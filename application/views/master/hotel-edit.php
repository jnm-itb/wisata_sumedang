...
$harga = in($_POST['harga']);
	...
	$row = $this->model->row("hotel","judul='$judul' and whatsapp='$whatsapp' and id<>'$id'  ");
	if($row >= 1){
		$alert = "danger";
		...
		?> 
		<script>  document.location.href="<?php echo($site) ?>master/hotel";   </script>  
		<?php 
		exit();
	}
}  
...
<div class="card-body shadow-sm">
	<form method="post" enctype="multipart/form-data"> 
		...
		<input type="text" required class="form-control" name="judul" value="<?php echo($data->judul) ;  ?>" placeholder="Nama Hotel"    />
		<br />
		<span> Image </span> 
		...
		<input type="number" required class="form-control" name="harga" value="<?php echo($data->harga) ;  ?>" placeholder=""    />
		<br />
